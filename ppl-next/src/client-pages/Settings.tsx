          ...
          {bwLatest && (
            <div className={styles.bwCurrent}>
              ...
              <div className={styles.bwMeta}>
                ...
                {bwChange !== null && (
                  <span style={{ color: bwChange < 0 ? 'var(--success)' : 'var(--danger)', marginLeft: 8 }}>
                    {bwChange > 0 ? '+' : ''}{bwChange} lbs
                  </span>
                )}
              </div>
            </div>
        ...
        <section className={styles.section}>
          ...
          <button className={`btn btn-primary ${styles.measureSaveBtn}`}
            ...
            onClick={async () => {
              ...
              Object.entries(measurements).forEach(([k, v]) => { if (v) clean[k] = parseFloat(v as string) })
            ...
            {photos.slice(0, 9).map(p => (
              <div key={p.id} className={styles.photoThumb}>
                <img src={p.public_url} alt={p.date} className={styles.photoImg}
                  onError={e => { (e.target as HTMLImageElement).style.background = 'var(--bg3)' }} />
