...
export function useLastSession(dayKey: string) {
  ...
  const load = async () => {
    ...
    (Object.entries(byExercise) as [string, any][]).forEach(([exId, ex]: [string, any]) => {
      ...
      ex.sessionHistory = (Object.entries(grouped) as [string, any[]][])
