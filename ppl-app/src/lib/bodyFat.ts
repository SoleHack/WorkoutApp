...
export function bfCategory(bf: number | null, sex: string): { label: string; color: string } | null {
  ...
  if (sex === 'female') {
    if (bf < 10) return { label: 'Essential Fat', color: '#F43F5E' }
    if (bf < 20) return { label: 'Athletic',      color: '#3B82F6' }
    if (bf < 25) return { label: 'Fitness',        color: '#22C55E' }
    if (bf < 32) return { label: 'Average',        color: '#F97316' }
    return              { label: 'Above Average',  color: '#F43F5E' }
  } else {
    if (bf < 5)  return { label: 'Essential Fat', color: '#F43F5E' }
    if (bf < 13) return { label: 'Athletic',      color: '#3B82F6' }
    if (bf < 18) return { label: 'Fitness',        color: '#22C55E' }
    if (bf < 25) return { label: 'Average',        color: '#F97316' }
    return              { label: 'Above Average',  color: '#F43F5E' }
  }
}
